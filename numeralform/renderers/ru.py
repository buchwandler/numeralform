...
from ..model import DecimalNumber, DigitSequence, FractionNumber, Gender, NumeralForm, NumeralRequest, NumeralResult, Syntax
from .base import validate_request

_UNDER_20 = ("ноль", "один", "два", "три", "четыре", "пять", "шесть", "семь", "восемь", "девять", "десять", "одиннадцать", "двенадцать", "тринадцать", "четырнадцать", "пятнадцать", "шестнадцать", "семнадцать", "восемнадцать", "девятнадцать")
...
_ONES = {
    ...
    4: {
        "m": ("четыре", "четырёх", "четырём", ("четырёх", "четыре"), "четырьмя", "четырёх"),
        "f": ("четыре", "четырёх", "четырём", ("четырёх", "четыре"), "четырьмя", "четырёх"),
...
_HUNDRED_CASES = {
    1: ("сто", "ста", "ста", "сто", "ста", "ста"), 2: ("двести", "двухсот", "двумстам", "двести", "двумястами", "двухстах"),
    3: ("триста", "трёхсот", "трёмстам", "триста", "тремястами", "трёхстах"), 4: ("четыреста", "четырёхсот", "четырёмстам", "четыреста", "четырьмястами", "четырёхстах"),
    5: ("пятьсот", "пятисот", "пятистам", "пятьсот", "пятьюстами", "пятистах"), 6: ("шестьсот", "шестисот", "шестистам", "шестьсот", "шестьюстами", "шестистах"),
    7: ("семьсот", "семисот", "семистам", "семьсот", "семьюстами", "семистах"), 8: ("восемьсот", "восьмисот", "восьмистам", "восемьсот", "восемьюстами", "восьмистах"),
    9: ("девятьсот", "девятисот", "девятистам", "девятьсот", "девятьюстами", "девятистах"),
}
_SCALE_NOM = {3: ("тысяча", "тысячи", "тысяч"), 6: ("миллион", "миллиона", "миллионов"), 9: ("миллиард", "миллиарда", "миллиардов"), 12: ("триллион", "триллиона", "триллионов"), 15: ("квадриллион", "квадриллиона", "квадриллионов"), 18: ("квинтиллион", "квинтиллиона", "квинтиллионов")}
_SCALE_CASES = {
    "тысяча": (("тысяча", "тысячи", "тысяч"), ("тысячи", "тысяч", "тысяч"), ("тысяче", "тысячам", "тысячам"), ("тысячу", "тысячи", "тысяч"), ("тысячей", "тысячами", "тысячами"), ("тысяче", "тысячах", "тысячах")),
    "миллион": (("миллион", "миллиона", "миллионов"), ("миллиона", "миллионов", "миллионов"), ("миллиону", "миллионам", "миллионам"), ("миллион", "миллиона", "миллионов"), ("миллионом", "миллионами", "миллионами"), ("миллионе", "миллионах", "миллионах")),
    "миллиард": (("миллиард", "миллиарда", "миллиардов"), ("миллиарда", "миллиардов", "миллиардов"), ("миллиарду", "миллиардам", "миллиардам"), ("миллиард", "миллиарда", "миллиардов"), ("миллиардом", "миллиардами", "миллиардами"), ("миллиарде", "миллиардах", "миллиардах")),
    "триллион": (("триллион", "триллиона", "триллионов"), ("триллиона", "триллионов", "триллионов"), ("триллиону", "триллионам", "триллионам"), ("триллион", "триллиона", "триллионов"), ("триллионом", "триллионами", "триллионами"), ("триллионе", "триллионах", "триллионах")),
}
_ORD_STEMS = {0: ("нулев", "ой"), 1: ("перв", "ый"), 2: ("втор", "ой"), 3: ("треть", "ий"), 4: ("четвёрт", "ый"), 5: ("пят", "ый"), 6: ("шест", "ой"), 7: ("седьм", "ой"), 8: ("восьм", "ой"), 9: ("девят", "ый"), 10: ("десят", "ый"), 11: ("одиннадцат", "ый"), 12: ("двенадцат", "ый"), 13: ("тринадцат", "ый"), 14: ("четырнадцат", "ый"), 15: ("пятнадцат", "ый"), 16: ("шестнадцат", "ый"), 17: ("семнадцат", "ый"), 18: ("восемнадцат", "ый"), 19: ("девятнадцат", "ый")}


def _case_index(case) -> int:
    value = getattr(case, "value", case) or "nominative"
    return {"nominative": 0, "genitive": 1, "dative": 2, "accusative": 3, "instrumental": 4, "prepositional": 5, "locative": 5}.get(value, 0)
...
class RussianRenderer:
    ...
    def capabilities() -> LocaleCapabilities:
        cases = frozenset({"nominative", "genitive", "dative", "accusative", "instrumental", "prepositional", "locative"})
        common = dict(cases=cases, animacies=frozenset({"animate", "inanimate"}), grammatical_numbers=frozenset({"singular", "plural"}), genders=frozenset({Gender.MASCULINE, Gender.FEMININE, Gender.NEUTER}))
        return LocaleCapabilities(profiles=(
            CapabilityProfile(NumeralForm.CARDINAL, syntaxes=frozenset({Syntax.STANDALONE, Syntax.ATTRIBUTIVE}), domain=NumericDomain(maximum=10**18 - 1), **common),
            CapabilityProfile(NumeralForm.ORDINAL, syntaxes=frozenset({Syntax.STANDALONE, Syntax.ORDINAL_ADJECTIVAL}), domain=NumericDomain(minimum=0, maximum=10**18 - 1, allow_negative=False), **common),
            CapabilityProfile(NumeralForm.DIGITS), CapabilityProfile(NumeralForm.DECIMAL), CapabilityProfile(NumeralForm.FRACTION), CapabilityProfile(NumeralForm.YEAR),
        ), notes=("Russian case, gender, number, and animacy inventories are explicit.", "Scale support covers thousand through quintillion."))
    ...
    def _cardinal(self, value: int, morphology) -> str:
        ...
        gender = _gender_key(morphology.gender, morphology.grammatical_number == "plural")
    ...
    def _scale(self, group: int, name: str, case: int) -> str:
        ...
        if forms is None:
            return name
        category = 0 if group % 10 == 1 and group % 100 != 11 else 1 if group % 10 in (2, 3, 4) and not 12 <= group % 100 <= 14 else 2
    ...
    def _ordinal(self, value: int, morphology) -> str:
        ...
        if value < 20:
            if value == 3:
                special = {
                    "m": ("третий", "третьего", "третьему", "третьего" if animate else "третий", "третьим", "третьем"),
                    "f": ("третья", "третьей", "третьей", "третью", "третьей", "третьей"),
                    "n": ("третье", "третьего", "третьему", "третье", "третьим", "третьем"),
                    "p": ("третьи", "третьих", "третьим", "третьих" if animate else "третьи", "третьими", "третьих"),
                }
        ...
        for exponent in sorted(_SCALE_NOM, reverse=True):
            ...
            if value >= scale:
                ...
                return self._integer(group, 1, "m", animate) + self._ordinal_scale(scale_name, case, gender)
        hundreds, remainder = divmod(value, 100)
        prefix = _HUNDRED_CASES[hundreds][0]
        return f"{prefix} {self._ordinal(remainder, morphology)}" if remainder else self._ordinal_hundred(hundreds, case, gender, animate)
    ...
    def _ordinal_word(self, value: int, case: int, gender: str, animate: bool) -> str:
        ...
        endings = {"m": (ending, "ого", "ому", "ого" if animate else ending, "ым", "ом"), "f": ("яя" if stem.endswith("ь") else "ая", "ей" if stem.endswith("ь") else "ой", "ей" if stem.endswith("ь") else "ой", "юю" if stem.endswith("ь") else "ую", "ей" if stem.endswith("ь") else "ой", "ей" if stem.endswith("ь") else "ой"), "n": ("ее" if stem.endswith("ь") else "ое", "его" if stem.endswith("ь") else "ого", "ему" if stem.endswith("ь") else "ому", "ее" if stem.endswith("ь") else "ое", "им" if stem.endswith("ь") else "ым", "ем" if stem.endswith("ь") else "ом"), "p": ("ие" if stem.endswith("ь") else "ые", "их", "им", "их" if animate else ("ие" if stem.endswith("ь") else "ые"), "ими", "их")}
        return stem + endings[gender][case]

    def _ordinal_tens(self, tens: int, case: int, gender: str, animate: bool) -> str:
        stems = {2: "двадцат", 3: "тридцат", 4: "сороков", 5: "пятидесят", 6: "шестидесят", 7: "семидесят", 8: "восьмидесят", 9: "девяност"}
        return self._ordinal_from_stem(stems[tens], case, gender, animate, "ый" if tens not in (4, 9) else "ой")

    def _ordinal_from_stem(self, stem, case, gender, animate, ending):
        return self._ordinal_word(1, case, gender, animate).replace("первый", stem + ending) if case == 0 and gender == "m" else stem + {"m": (ending, "ого", "ому", "ого", "ым", "ом"), "f": ("ая", "ой", "ой", "ую", "ой", "ой"), "n": ("ое", "ого", "ому", "ое", "ым", "ом"), "p": ("ые", "ых", "ым", "ые", "ыми", "ых")} [gender][case]

    def _ordinal_scale(self, name, case, gender):
        stem = name[:-1] if name.endswith("а") else name
        return self._ordinal_from_stem(stem + "н", case, gender, False, "ый")

    def _ordinal_hundred(self, hundreds, case, gender, animate):
        stem = {1: "сот", 2: "двухсот", 3: "трёхсот", 4: "четырёхсот", 5: "пятисот", 6: "шестисот", 7: "семисот", 8: "восьмисот", 9: "девятисот"}[hundreds]
    ...
    def _digits(self, value) -> str:
        ...
        return ("минус " if negative else "") + " ".join(_DIGITS[int(d)] for d in digits)
    ...
    def _decimal(self, value) -> str:
        ...
        return f"{sign}{self._integer(int(value.integer), 0, 'm', False)} точка " + " ".join(_DIGITS[int(d)] for d in value.fraction)
    ...
    def _fraction(self, value) -> str:
        ...
        denominator = {2: "вторая", 3: "третья", 4: "четвёртая", 5: "пятая", 6: "шестая", 7: "седьмая", 8: "восьмая", 9: "девятая", 10: "десятая"}.get(value.denominator, self._ordinal(value.denominator, Morphology(gender=Gender.FEMININE)))
