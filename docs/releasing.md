...
Numeralform uses `setuptools-scm`. The Git tag is the release version source of truth; there is no manually maintained package version.

Release tags use the form `v<version>`, for example `v0.1.2`. Building from that tag must produce package version `0.1.2`. `numeralform/_version.py` is generated by `setuptools-scm` and must not be committed. A raw source checkout without generated version metadata falls back to `0+unknown`.

Before publishing a release:

1. make sure the release commit has passed CI and benchmark gates;
2. create and push the release tag;
3. publish the GitHub Release for that tag;
4. let the publish workflow build directly from the tag;
5. require the built wheel, sdist, and runtime versions to match the tag.
...
Untagged development checkouts can still be built. Their version is calculated by `setuptools-scm` and should not be hard-coded.
...
The publish workflow validates the release tag, built artifacts, installed metadata, and runtime version before uploading to PyPI.
