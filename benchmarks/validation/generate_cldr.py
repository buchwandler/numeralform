...
CONFIG_PATH = CONFIG_ROOT / "cldr.toml"
CLDR_CORPUS = DATA_ROOT / "corpora" / "cldr"
if __package__ in {None, ""}:  # support the documented ``python tools/...py`` form
    sys.path.insert(0, str(Path(__file__).resolve().parents[2]))
    from benchmarks.validation.cases import case_id, deterministic_values
    from benchmarks.validation.corpus import manifest_for, write_jsonl, write_manifest
    from benchmarks.validation.mapping import Mapping, load_config, mappings
    from benchmarks.validation.model import (
        SerializedValue,
        ValidationCase,
        ValidationRequest,
    )
    from benchmarks.validation.normalize import normalize_nfc
    from benchmarks.validation.oracle.icu import ICUOracle, OracleUnavailable
else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output", type=Path, default=CLDR_CORPUS)
