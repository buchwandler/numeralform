...
def shared_locale_pairs(
    ...
    *,
    ...
    canonical = {normalize_locale(locale) for locale in (canonical_locales or numeralform.locales())}
...
def _edge_values() -> tuple[int, ...]:
    return (
        ...
        29, 30, 31, 32, 33, 99, 100, 101, 102, 103, 109, 110, 111,
        112, 113, 119, 120, 121, 122, 123, 199, 200, 201, 999, 1000,
        1001, 1002, 1003, 1999, 2000, 2001, 9999, 10000, 10001,
        99999, 100000, 100001, 999999, 1000000, 1000001,
    )
...
def _year_edge_values() -> tuple[int, ...]:
    return (
        0, 1, 9, 10, 11, 50, 99, 100, 101, 599, 600, 601, 899, 900,
        905, 999, 1000, 1001, 1009, 1010, 1099, 1100, 1101, 1899, 1900,
        1901, 1909, 1910, 1999, 2000, 2001, 2009, 2010, 2024, 2099, 2100,
        2101, 2999, 3000, 3001, 5555, 6600, 9999,
    )


def _decimal_edge_values() -> tuple[Decimal, ...]:
    return tuple(Decimal(value) for value in (
        "0.0", "0.00", "0.01", "0.001", "0.10", "0.20", "1.0", "1.00",
        "1.01", "1.10", "1.20", "9.99", "10.01", "10.10", "99.99",
        "100.01", "1000.01",
    ))
...
def _currency_edge_values(minor_units: int) -> tuple[Decimal, ...]:
    if minor_units == 0:
        return tuple(Decimal(value) for value in ("0", "1", "2", "4", "5", "10", "11", "12", "14", "20", "21", "22", "99", "100", "101"))
    return tuple(Decimal(value) for value in (
        "0.00", "0.01", "0.02", "0.04", "0.05", "0.10", "0.11", "0.12", "0.14",
        "0.20", "0.21", "0.22", "0.99", "1.00", "1.01", "1.02", "1.11", "2.00",
        "2.01", "4.01", "5.01", "10.00", "11.00", "12.00", "14.00", "20.00",
        "21.00", "22.00", "101.01",
    ))


def _maybe_negative(rng: random.Random, value: int | Decimal, probability: float) -> int | Decimal:
    if value == 0:
        return value
...
def _decimal(rng: random.Random, profile: dict) -> Decimal:
    ...
    return _maybe_negative(rng, value, float(profile.get("decimal_negative_probability", profile.get("negative_probability", 0.0))))
...
def _currency_decimal(rng: random.Random, profile: dict, currency: str) -> Decimal:
    ...
    if rng.random() < float(profile.get("currency_edge_probability", profile.get("edge_probability", 0.0))):
        candidates = [value for value in _currency_edge_values(digits) if minimum <= value <= maximum]
        ...
        if digits == 0:
            ...
            value = Decimal(f"{integer}.{fraction:0{digits}d}")
    return _maybe_negative(rng, value, float(profile.get("currency_negative_probability", 0.0)))
...
def _canonical_options(rng: random.Random, locale: str, kind: str, config: RandomConfig) -> tuple[dict[str, str | int | bool], str | None]:
    ...
    if language == "ru" and kind in {"cardinal", "ordinal"}:
        matrix = (
            {"gender": "masculine", "case": "nominative", "plural": False, "animate": False},
            {"gender": "feminine", "case": "nominative", "plural": False, "animate": False},
            {"gender": "masculine", "case": "genitive", "plural": False, "animate": False},
            {"gender": "masculine", "case": "dative", "plural": False, "animate": False},
            {"gender": "masculine", "case": "accusative", "plural": False, "animate": True},
            {"gender": "masculine", "case": "accusative", "plural": False, "animate": False},
            {"gender": "feminine", "case": "instrumental", "plural": False, "animate": False},
            {"gender": "masculine", "case": "prepositional", "plural": False, "animate": False},
            {"plural": True, "case": "nominative", "animate": False},
        )
        selected = rng.choice(matrix)
        return selected, f"ru-{kind}-" + "-".join(str(value) for value in selected.values())
    return {}, None
...
def _candidate(
    ...
    *,
    ...
    if kind in {"cardinal", "ordinal", "ordinal_num", "year"}:
        if kind == "cardinal":
            minimum, maximum, edges = int(profile["cardinal_min"]), int(profile["cardinal_max"]), _edge_values()
            value = random_integer(rng, minimum=minimum, maximum=maximum, edge_values=edges, edge_probability=edge_probability)
            if minimum >= 0:
                value = _maybe_negative(rng, value, float(profile.get("negative_probability", 0.0)))
        elif kind in {"ordinal", "ordinal_num"}:
            minimum, maximum, edges = int(profile["ordinal_min"]), int(profile["ordinal_max"]), _ordinal_edge_values()
            value = random_integer(rng, minimum=minimum, maximum=maximum, edge_values=edges, edge_probability=edge_probability)
        else:
            minimum, maximum, edges = int(profile["year_min"]), int(profile["year_max"]), _year_edge_values()
            value = random_integer(rng, minimum=minimum, maximum=maximum, edge_values=edges, edge_probability=edge_probability)
        options, variant_id = _canonical_options(rng, locale, kind, config) if target == "canonical" else ({}, None)
        if target == "compat":
...
def supported_currency_candidates(*, locale: str, oracle_locale: str, configured: tuple[str, ...], currency_supports: Callable[..., bool], oracle_supports: Callable[..., bool]) -> tuple[str, ...]:
    probe = Decimal("1.01")
    return tuple(
        code
...
def _default_supports(locale: str, kind: str, value: int | Decimal, **options) -> bool:
    ...
    syntax = "attributive" if locale.split("-", 1)[0] == "es" and kind == "ordinal" and "gender" in options else "standalone"
...
def generate_cases(
    *,
    ...
    unknown_currencies = set(selected_currencies) - set(profile_values["currency_minor_units"])
    if unknown_currencies:
        raise ValueError("currency minor units are not configured: " + ", ".join(sorted(unknown_currencies)))
    ...
    max_attempts = max(count * int(profile_values.get("max_attempt_multiplier", 30)), count + 1)
    attempts = 0
    ...
    if len(generated) != count:
        raise RuntimeError(f"unable to generate {count} supported unique cases after {attempts} attempts")
