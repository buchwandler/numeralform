...
def summarize(
    ...
    *,
    ...
    summary["comparability"] = {
        ...
        "exact_parity_rate": counts.get("match", 0) / comparable_cases
        if comparable_cases
        else 0.0,
        "semantic_parity_rate": semantic_matches / comparable_cases
        if comparable_cases
        else 0.0,
    }
    ...
    oracle_errors = tuple(
        result for result in results if result.status == "oracle-error"
    )
    ...
    summary["breakdowns"] = {
        ...
        "options_by_status": dict(
            sorted(
                Counter(
                    f"{_option_key(result)}|{result.status}" for result in results
                ).items()
            )
        ),
        "transport": dict(
            sorted(Counter(result.case.transport for result in results).items())
        ),
        "option_profile_id": dict(
            sorted(
                Counter(
                    result.case.option_profile_id or "<default>" for result in results
                ).items()
            )
        ),
        "differences_by_shape": dict(
            sorted(
                Counter(
...
def human_report(
    ...
    *,
    ...
    for label, values in (
        ...
        (
            "mismatches by oracle locale",
            summary["breakdowns"]["mismatches_by_oracle_locale"],
        ),
        ...
        (
            "differences by difference shape",
            summary["breakdowns"]["differences_by_shape"],
        ),
        (
            "mismatches by difference shape",
            summary["breakdowns"]["mismatches_by_shape"],
        ),
...
def write_reports(
    ...
    *,
    ...
    report_path.write_text(
        human_report(results, metadata=metadata, output_path=str(differences_path)),
        encoding="utf-8",
    )
    paths = {
